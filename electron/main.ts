  ...
  if (store.get("firsti") == undefined) {
    ...
    fs.mkdir(path.join(__dirname, "../instances"), (err: any) => {
      ...
      fs.mkdir(path.join(__dirname, "../instances/" + name), (err: any) => {
        ...
        if (client == "forge") {
          download("https://maven.minecraftforge.net/net/minecraftforge/forge/1.19.2-43.2.14/forge-1.19.2-43.2.14-installer.jar", path.join(__dirname, "../instances/" + name + "/forge.jar"))
        } else if (client == "fabric") {
